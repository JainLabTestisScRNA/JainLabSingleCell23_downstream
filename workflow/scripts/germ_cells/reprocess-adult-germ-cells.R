...
#reducedDim(sce) <- reducedDim(sce,"HARMONY")[,c(1,2)]

set.seed(2)
nn.clust <- clusterCells(sce,use.dimred = "corrected", full=TRUE,
                         BLUSPARAM=KmeansParam(12))

colLabels(sce) <- nn.clust$clusters |> as.character()

plotReducedDim(sce[,order(sce$label)], dimred = "corrected", ncomponents = c(1,3),colour_by = "label",text_by = "label") + paletteer::scale_color_paletteer_d("ggsci::default_igv")
...
plotReducedDim(sce, dimred = "corrected", ncomponents = c(1,3),colour_by = "label",text_by = "label") + paletteer::scale_color_paletteer_d("ggsci::default_igv")


order_df <- tibble(label=(as.character(c(5,10,3,2,6,7,1,12,9,4,8,11)))) |>
