...
```{r}
sce_fl <- "results/single-cell-preproc/count/import/adult1.cellranger.sce.rds"

raw_fl <- "results/single-cell-preproc/count/raw/adult1.cellranger.sce.rds"

juv_fl <- "~/work/JainLabSingleCell23_downstream/results/single-cell-preproc/import/juvenile_13d_wt_null.cellranger.sce.rds"

juv_raw_fl <- "~/work/JainLabSingleCell23_downstream/results/single-cell-preproc/raw/juvenile_13d_wt_null.cellranger.sce.rds"
```
...
sce <- read_rds(sce_fl)
raw <- read_rds(raw_fl)
juv_raw <- read_rds(juv_raw_fl)
```

## depth

too much to plot everything, but will randomly select 2000 cells to plot. So using cellranger called cells

```{r}
plot_sampled_hist <- \(x) x |>
  addPerCellQCMetrics() |>
  colData() |>
  as_tibble() |>
  ggplot(aes(sum)) +
...
plot_sampled_hist(sce) /
plot_sampled_hist(juv)
```

## all kneeplots

```{r}
plot_knee <- function(x) {
  map_df(x, ~as_tibble(DropletUtils::barcodeRanks(counts(.x))),.id="sample") -> bcrank
  
  as_tibble(bcrank) |>
  dplyr::select(rank, total,sample) |>
  distinct() |>
  ggplot(aes(rank,total, color=sample)) +
  geom_line() +
  scale_y_log10() +
  scale_x_log10() +
...
```{r}
raw_rdss <- list(adult1=read_rds("results/single-cell-preproc/count/raw/adult1.cellranger.sce.rds"),
     adult2 = read_rds("results/single-cell-preproc/count/raw/adult2.cellranger.sce.rds"),
                        adult3=read_rds("results/single-cell-preproc/count/raw/adult3.cellranger.sce.rds"),
                                        adult4=read_rds("results/single-cell-preproc/count/raw/adult4.cellranger.sce.rds"),
                                                         juv = juv_raw)

bcranks <- map(raw_rdss, ~DropletUtils::barcodeRanks(counts(.x)),.id="sample")

df <- bcranks |> map_df(as_tibble,.id="sample")
...
```{r}
g_kneeplots1 <- plot_knee(read_rds("results/single-cell-preproc/count/raw/adult1.cellranger.sce.rds"))

g_kneeplots2 <- plot_knee(read_rds("results/single-cell-preproc/count/raw/adult2.cellranger.sce.rds"))
g_kneeplots3 <- plot_knee(read_rds("results/single-cell-preproc/count/raw/adult3.cellranger.sce.rds"))
g_kneeplots4 <- plot_knee(read_rds("results/single-cell-preproc/count/raw/adult4.cellranger.sce.rds"))
```

```{r}
g_juv_kneeplot <- plot_knee(juv_raw)

 g_kneeplots1 + g_kneeplots2 + g_juv_kneeplot + ggtitle("juvenile") + g_kneeplots3 + g_kneeplots4 + plot_layout(ncol=3)
